...
class DAVHandler(HttpOptions, DAVRequestHandler, FixSendError):
    verbose = False
    ...
    _HTTP_OPTIONS= { 'DAV' : ['1', '2'],
                    'Allow' : [ 'GET', 'HEAD', 'COPY', 'MOVE', 'POST', 'PUT',
                            'PROPFIND', 'PROPPATCH', 'OPTIONS', 'MKCOL',
                                'DELETE', 'TRACE', 'REPORT', 'LOCK', 'UNLOCK']
                    }
    ...
    def _flush(self):
        self.IFACE_CLASS.clean_up_connection()
    ...
    def send_response(self, code, message=None):
        ...
        if self.command == 'PROPFIND' and int(code) == 404:
            ...
            if kh and (kh.group(1) < '4.5'):
                ...
                code = 200
        if self.command == "GET":
            BaseHTTPRequestHandler.default_request_version = "HTTP/1.1"
...
class DAVAuthProvider(AuthProvider):
    ...
    def checkRequest(self,handler,path, db=False):        
        ...
        if auth_str == '' or auth_str == False:
            cookie = handler.headers.get('Cookie', False)
            if not cookie == False:
                cookies = cookie.split(";")
                for c in cookies:
                    if c.strip().startswith("odoo_auth"):
                        cv = c.split("=")
                        auth_str = cv[1]
